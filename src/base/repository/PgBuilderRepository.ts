...
class PgBuilderRepository {
  ...
  protected upsert_query(tableName: string, args: Record<string, any>, returning = '*'): this {
    ...
    if (id != 0) return this.update(tableName, id, sanitizedArgs, returning)
  ...
  protected select(selectArg?: string | string[]): this {
    switch (typeof selectArg) {
      ...
      case 'object':
        this.selectQuery = /* SQL */ `SELECT ${selectArg
  ...
  private generateAllQueryAndParams(): { query: string; params: any[] } {
    ...
    if (this.queryString || this.queryParams.length) this.params = this.queryParams
      ...
      if (this.leftOuterJoinParams.length)
        this.params = [...this.params, ...this.leftOuterJoinParams]
      if (this.rightOuterJoinParams.length)
        this.params = [...this.params, ...this.rightOuterJoinParams]
      if (this.fullOuterJoinParams.length)
        this.params = [...this.params, ...this.fullOuterJoinParams]
